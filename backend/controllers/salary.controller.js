...
exports.generate = async (req, res) => {
  ...
  for (const worker of workers) {
    const records = await Attendance.find({ worker: worker._id, date: { $regex: `^${month}` } });
    ...
    const netSalary     = grossSalary;  // Extend: add deductions logic here

    const salary = await Salary.findOneAndUpdate(
      { worker: worker._id, month },
      { worker: worker._id, month, daysPresent, daysHalfDay, daysAbsent, dailyWage: worker.dailyWage, grossSalary, netSalary },
...
exports.getByMonth = async (req, res) => {
  ...
  const records = await Salary.find({ month }).populate("worker", "name role phone dailyWage");
  const totalPayroll = records.reduce((sum, r) => sum + r.netSalary, 0);
  const totalPaid    = records.filter(r => r.paymentStatus === "Paid").reduce((sum, r) => sum + r.netSalary, 0);

  res.json({ success: true, count: records.length, totalPayroll, totalPaid, data: records });
};
